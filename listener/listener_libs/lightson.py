#requests.post("http://192.168.0.111:8080/json.htm?type=command&param=switchscene&idx=3&switchcmd=On&level=0")
def start():
    ...
    for device in data['result']:
        if ('Light' in device['Image'] and (('light' in device['TypeImg']) or ('dimmer' in device['TypeImg']))):
            ...
            if (device['Status']=='Off'):
                requests.post("http://192.168.0.111:8080/json.htm?type=command&param=switchlight&switchcmd=On&idx="+device['idx'])